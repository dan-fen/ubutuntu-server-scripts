...
REAL_PATH=$(realpath /usr/local/bin/archive-home)
...
BACKUP_DIR=$DEST_DIR/home-archive/$(date +%Y-%m-%d)
EXCLUDE_FILE=$SCRIPT_DIR/exclude-list-home.txt

# --- Functions ---
header() {
    echo -e "\n=========================\nHOME ARCHIVE STARTING NOW\n=========================" | tee -a "$LOG_FILE"
...
archive_backup() {
    tar -cpf - --exclude-from="$EXCLUDE_FILE" -C "$SOURCE_DIR" . 2>> "$LOG_FILE" | gzip 2>> "$LOG_FILE" > "$BACKUP_DIR/home-backup-archive.tar.gz"
